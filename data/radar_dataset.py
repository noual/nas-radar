...
class RadarDavaTestDataset(RadarDavaDataset):
    ...
    def __getitem__(self, idx):
        ...
        x_dava = preprocess_dava_card(x, None, None, random_crops=random_crops, flip=flip)
        y_dava = preprocess_dava_card(y, None, None, label=True, random_crops=random_crops, flip=flip)
        x = torch.from_numpy(x_dava).float()
        y = torch.from_numpy(y_dava).float()
        return x, y
