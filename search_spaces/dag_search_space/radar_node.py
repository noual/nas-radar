...
class RadarNode:
    ...
    def from_str(self, arch_str: str) -> None:
        """
        Parse a string representation to set the architecture choices.
        
        Args:
            arch_str: String in the format produced by to_str().
        """
        channel_part, cell_part = arch_str.split('|')
        channel_values = channel_part.split(':')
        
        # Parse encoder channels
        for i in range(self.num_encoder_stages):
            ch_str = channel_values[i]
            self._encoder_channels[i] = int(ch_str) if ch_str != '?' else None
        
        # Parse bottleneck channels
        btl_str = channel_values[self.num_encoder_stages]
        self._bottleneck_channels = int(btl_str) if btl_str != '?' else None
        
        # Parse decoder channels
        for i in range(self.num_decoder_stages):
            ch_str = channel_values[self.num_encoder_stages + 1 + i]
            self._decoder_channels[i] = int(ch_str) if ch_str != '?' else None
        
        # Parse cell operations
        op_map_reverse = {
            'c1x1': 'conv_1x1',
            'c1x3': 'conv_1x3_3x1',
            'dw3x3': 'depthwise_conv_3x3',
            'c1x5': 'conv_1x5_5x1',
            'd3r2': 'dilated_conv_3x3_r2',
            'd3r4': 'dilated_conv_3x3_r4',
            'id': 'identity',
            'z': 'none',
        }
        
        op_values = cell_part.split(':')
        edge_names = self._get_edge_names()
        
        for edge_name, op_str in zip(edge_names, op_values):
            if op_str != '?':
                self._cell_operations[edge_name] = op_map_reverse.get(op_str, op_str)
            else:
                self._cell_operations[edge_name] = None
