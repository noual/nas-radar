...
OPERATIONS = {
    ...
    'conv_1x3_3x1': lambda in_ch, out_ch: nn.Sequential(
        nn.Conv2d(in_ch, out_ch, kernel_size=(1, 3), padding=(0, 1), bias=False),
        nn.BatchNorm2d(out_ch),
        nn.Conv2d(out_ch, out_ch, kernel_size=(3, 1), padding=(1, 0), bias=False),
        nn.BatchNorm2d(out_ch),
        nn.ReLU(inplace=True)
    ),
    ...
    'conv_1x5_5x1': lambda in_ch, out_ch: nn.Sequential(
        nn.Conv2d(in_ch, out_ch, kernel_size=(1, 5), padding=(0, 2), bias=False),
        nn.BatchNorm2d(out_ch),
        nn.BatchNorm2d(out_ch),
        nn.ReLU(inplace=True)
    ),
    ...
    'dilated_conv_3x3_r2': lambda in_ch, out_ch: nn.Sequential(
        nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=2, bias=False, dilation=2),
        nn.BatchNorm2d(out_ch),
        nn.ReLU(inplace=True)
    ),
    'dilated_conv_3x3_r4': lambda in_ch, out_ch: nn.Sequential(
        nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=4, bias=False, dilation=4),
        nn.BatchNorm2d(out_ch),
        nn.ReLU(inplace=True)
    ),
